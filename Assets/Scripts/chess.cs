...
public class chess : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        tree = new GameTree(15, 15);
        Debug.Log("build GameTree");
        last1 = new node(7, 7);
        last2 = new node(7, 7);

        node x = tree.Cal(1, chessState, last1, last2);
        Debug.Log(x.x);
    ...
    void Update()
    {
        ...
        {
            node pos = tree.Cal(-1, chessState, last1, last2);
            //根据下棋顺序确定落子颜色
            chessState[pos.x, pos.y] = chessTurn == turn.black ? 1 : -1;
            //落子成功，更换下棋顺序
            chessTurn = chessTurn == turn.black ? turn.white : turn.black;
            last2 = last1;
            last1 = new node(pos.x, pos.y);
            //调用判断函数，确定是否有获胜方
            int re = result();
            if (re == 1)
            {
